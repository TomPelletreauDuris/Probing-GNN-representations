...
# ## Visualisation
# 
# For visualisation, we take into account the disconnected subgraph 
...
# Assuming test_node_properties[0][0] contains the betweenness centrality values for the nodes
node_degrees = [node['degree'] for node in test_node_properties[0]]


plt.figure(figsize=(10, 10))
nx.draw(G, with_labels=True, node_size=[v * 100 for v in node_degrees], node_color=node_degrees, cmap='viridis')

#save in svg 
plt.savefig('results/'+DATASET+"_"+MODEL+'_graph_degree.svg', dpi=300, bbox_inches='tight', format='svg')
plt.show()

# %%
import matplotlib.pyplot as plt
#represent the properties of betweenness centrality on the graph by chaging the size of the nodes
betweenness_centrality = [node['betweenness'] for node in test_node_properties[0]]
node_degrees = [node['degree'] for node in test_node_properties[0]]
...
#save in svg
plt.savefig('results/'+DATASET+"_"+MODEL+'_graph_betweenness_centrality.svg', dpi=300, bbox_inches='tight', format='svg')
plt.show()


# %%
#same for eigenvector centrality
eigenvector_centralities = [node['eigenvector'] for node in test_node_properties[0]]

plt.figure(figsize=(10, 10))
nx.draw(G, with_labels=True, node_size=[v * 10000 for v in eigenvector_centralities], node_color=node_degrees, cmap='viridis')

#save in svg
plt.savefig('results/'+DATASET+"_"+MODEL+'_graph_eigenvector_centrality.svg', dpi=300, bbox_inches='tight', format='svg')
plt.show()

# %%
#same for local clustering coefficients
Local_clustering_coefficients = [node['clustering'] for node in test_node_properties[0]]

plt.figure(figsize=(10, 10))
nx.draw(G, with_labels=True, node_size=[v * 1000 for v in Local_clustering_coefficients], node_color=node_degrees, cmap='viridis')

#save in svg
plt.savefig('results/'+DATASET+"_"+MODEL+'_graph_local_clustering_coefficients.svg', dpi=300, bbox_inches='tight', format='svg')
plt.show()

# %% [markdown]
# There might be more discriminative features based on the eigeivector centrality. 
# 
...
# ## Node Properties 
# 
# For our probing purposes, we don't take into account the disconnected graphs 
