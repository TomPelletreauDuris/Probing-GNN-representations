...
from models.models_ClinTox import GAT_framework as framework # import the model
...
def compute_graph_properties(data):
    ...
    for idx, graph_data in enumerate(data):
        ...
        property_tuple = (num_nodes, num_edges, density, avg_path_len, num_cliques, num_triangles, num_squares, number_of_node_in_the_largest_fully_connected_component, assortativity, small_world)
        
        # Debug statement to check the length of the tuple
        if len(property_tuple) != 10:
            print(f"Graph ID {idx} has an incorrect number of properties: {len(property_tuple)}")
